...
export default function Header({ onClearChat, messageCount, onToggleSidebar }) {
  return (
    <header className="h-14 shrink-0 flex items-center justify-between px-4 lg:px-6 border-b border-white/5 bg-cosmic-800/40 backdrop-blur-xl">
      <div className="flex items-center gap-3">
        <motion.button
          onClick={onToggleSidebar}
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.9 }}
          className="p-2 -ml-1.5 rounded-xl hover:bg-white/5 text-cosmic-200 transition-colors"
        >
      ...
      <div className="flex items-center gap-1">
        {messageCount > 0 && (
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-semibold text-cosmic-200 hover:text-red-400 hover:bg-red-500/10 transition-all duration-150"
          >
